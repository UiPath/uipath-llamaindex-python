...
def _extract_agent_from_tool(tool: Any) -> Agent | None:
    ...
    try:
        ...
        if not callable(impl) or not hasattr(impl, "__closure__") or not impl.__closure__:
            return None

        # Second level: get run_agent function
        run_agent = impl.__closure__[1].cell_contents
        if not callable(run_agent) or not hasattr(run_agent, "__closure__") or not run_agent.__closure__:
            return None
        ...
        for tool in tools:
            if isinstance(tool, Agent):
                ...
                if wrapped_agent is not None:
                    ...
                    tool_name_str: str = (
                        _get_tool_name(tool) or getattr(wrapped_agent, "name", None) or "agent"
                    )
