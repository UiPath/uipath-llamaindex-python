...
@function_tool
async def check_budget_availability(department: str, amount: float, category: str = "General") -> str:
...
@function_tool
async def create_purchase_order(item: str, quantity: int, unit_price: float, vendor: str, department: str, justification: str) -> str:
    ...
    approval_level = "Manager" if grand_total < 5000 else "Director" if grand_total < 25000 else "VP + Finance"
...
async def track_order_status(po_number: str) -> str:
    ...
    import random
    statuses = [
        ("PENDING_APPROVAL", "Purchase order awaiting management approval", "N/A", "N/A"),
        ("APPROVED", "Order approved, sent to vendor", "TRK-2024-" + str(random.randint(10000, 99999)), "5-7 business days"),
        ("IN_TRANSIT", "Order shipped and in transit", "TRK-2024-" + str(random.randint(10000, 99999)), "2-3 business days"),
        ("DELIVERED", "Order delivered successfully", "TRK-2024-" + str(random.randint(10000, 99999)), "Delivered on 2024-01-25"),
    ]
...
@function_tool
async def request_budget_reallocation(from_category: str, to_category: str, amount: float, justification: str, department: str) -> str:
...
async def search_preferred_vendors(category: str) -> str:
    ...
    vendor_list = vendors_db.get(category, ["Contact Procurement for vendor recommendations"])

    return f"""Preferred Vendors - {category}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Approved vendor list for company purchases:

""" + "\n".join([f"  {i+1}. {v} (Contract on file)" for i, v in enumerate(vendor_list)]) + f"""
